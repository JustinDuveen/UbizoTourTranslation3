...
export function sendThroughDataChannel(
  ...
  if (!dataChannel || dataChannel.readyState !== 'open') {
      console.warn(`Cannot send ${type}: Data channel not open`);
      return false;
  }
...
function playAudioForGuide(stream: MediaStream, language?: string): void {
  ...
  try {
      ...
      // Add error handling
      audioEl.onerror = (e) => {
          console.error(`Guide audio playback error for ${language}:`, e);
          audioEl.remove();
          if (language) {
              guideAudioElements.delete(language);
          }
      ...
      try {
          ...
          console.error(`Error setting srcObject for guide audio (${language}):`, error);
          return;
      }
      ...
      console.error(`Error creating audio element for guide playback (${language}):`, error);
  }
...
*/
async function loadAudioInstructions(language: string): Promise<ArrayBuffer> {
...
function forwardTranslationToAttendees(language: string, translationData: any): void {
  ...
  for (const conn of connections) {
      if (conn.dc.readyState === "open") {
          try {
              console.log(`Sending translation data to attendee ${conn.id}:`, translationData);
...
function handleOpenAIMessage(e: MessageEvent, language: string, setTranslation: (translation: string) => void): void {
  ...
  if (realtimeEvent.type === "error") {
      console.error("OpenAI Server Error:", { ...realtimeEvent });
      return;
  }
  ...
  switch (realtimeEvent.type) {
      ...
      case "rate_limits.updated":
          console.log(`${realtimeEvent.type} event:`, realtimeEvent);
          break;

      case "response.audio.delta":
          // Not used with WebRTC - Audio is handled via ontack
          break;
      ...
      case "response.done":
       ...
       case "conversation.item.truncated":
            console.warn(`Conversation item truncated for ${language}:`, {
            ...
            // Optional: Trigger a warning to the guide about partial history
            break;
        ...
        console.log('Final transcript available for:', realtimeEvent.response_id);
        // Could trigger translation here
        break;

        case 'response.content_part.done':
        // Handle completed content segments
        break;

        case 'response.output_item.done':
        // Full output item is ready
        break;
      ...
      case "output_audio_buffer.started":
        console.log(`Audio playback started for ${language}`, realtimeEvent);
        // Optional: Start visual indicator that audio is playing
        break;
        
      case "output_audio_buffer.stopped":
        console.log(`Audio playback ended for ${language}`, realtimeEvent);
        // Optional: Stop visual indicator
        break;
        
      case "output_audio_buffer.cleared":
        console.log(`Audio buffer cleared for ${language}`, realtimeEvent);
        break;
      ...
      case "response.output_item.added":
          console.log("response.output_item.added event:", realtimeEvent);
          break;
      case "response.content_part.added":
          console.log("response.content_part.added event:", realtimeEvent);
          break;
      case "response.audio_transcript.delta":
          console.log("response.audio_transcript.delta event:", realtimeEvent);
          break;
      case "error":
        console.error("OpenAI Server Error:", {
            ...realtimeEvent,
            language,
            connectionState: openaiConnections.get(language)?.pc.connectionState
        });
        break;
...
async function setupOpenAIConnection(
  ...
  const openaiPC = new RTCPeerConnection({
    iceTransportPolicy: 'relay', // Force TURN only if NAT issues occur
      iceServers: [
          {
    ...
    const supportedCodecs = audioCapabilities?.codecs?.map(c => c.mimeType) || [];
    console.log("Supported audio codecs:", supportedCodecs);
  ...
  // Load audio instructions and prepend to the mic stream
  let audioInstructionsBuffer: ArrayBuffer | null = null;
  // Replace the audioInstructionsTrack section (around line 620) with:
if (audioInstructionsBuffer) {
    try {
      const audioBlob = new Blob([audioInstructionsBuffer], { type: 'audio/mp3' });
      ...
      audioElement.oncanplaythrough = () => {
        ...
        console.log("Adding processed audio instructions track");
        openaiPC.addTrack(destination.stream.getAudioTracks()[0]);
        
        audioElement.play().catch(e => console.error("Playback failed:", e));
      };
    } catch (error) {
      console.error("Alternative audio instruction handling failed:", error);
    }
  }

  // Create a MediaStreamTrack from the audio instructions (if loaded)
  ...
  if (audioInstructionsBuffer) {
      try {
          ...
          const audioBuffer = await audioContext.decodeAudioData(audioInstructionsBuffer);

          // Create a new MediaStreamTrack from the AudioBuffer
          const destination = audioContext.createMediaStreamDestination();
          ...
          console.log(`Created MediaStreamTrack from audio instructions`);
      } catch (error) {
          console.error(`Failed to create MediaStreamTrack from audio instructions:`, error);
      }
  }

  // Add the audio instructions track *before* the microphone track
  if (audioInstructionsTrack) {
      console.log("Adding audio instructions track to OpenAI connection");
  ...
  openaiDC.onopen = async () => {
      ...
      try {
          ...
          openaiDC.send(JSON.stringify({
            ...
            response: { 
              modalities: ["audio", "text"] // Keep only essential parameters
            }
          }));
        } catch (error) {
          console.error("Failed to send initial config:", error);
        }
      };

      openaiDC.onerror = (error) => {
        console.error(`Data channel error (${language}):`, error);
        
        // Attempt graceful reconnection
        setTimeout(() => {
          if (!openaiConnections.has(language)) {
            console.log(`Reconnecting ${language} after error...`);
  ...
  // 1. Validate the offer exists and has SDP
  if (!offer?.sdp) {
    throw new Error('Failed to create valid WebRTC offer');
  }
  
  // 2. Create defensive modification function
  const modifySDP = (sdp: string): string => {
    try {
      return sdp
        .replace(/a=recvonly/g, 'a=sendrecv')  // Force bidirectional
        .replace(/m=audio \d+/, '$& RTP/SAVPF 111') // Prefer Opus
        .replace(/a=mid:audio\r\n/g, 'a=mid:audio\r\na=sendrecv\r\n'); // Alternative approach
    } catch (e) {
      console.error('SDP modification failed, using original:', e);
  ...
  // 4. Verify modifications took effect
  if (!modifiedSDP.includes('a=sendrecv')) {
    console.warn('Failed to force bidirectional audio in SDP');
  }
  
  // 5. Set local description with validation
  try {
    await openaiPC.setLocalDescription({
      type: 'offer',
      sdp: modifiedSDP
    });
    console.log('Modified SDP set successfully');
  } catch (error) {
    console.error('Failed to set modified SDP, trying original:', error);
    // Fallback to unmodified SDP
  ...
  try {
      ...
      const openaiAnswer = await sdpResponse.text();
      console.log('OpenAI Answer SDP:', openaiAnswer);

      if (!openaiAnswer.includes('a=sendrecv')) {
        throw new Error('OpenAI answer does not support bidirectional audio');
      }
      ...
      console.log("Connection established, checking transceivers:");
        openaiPC.getTransceivers().forEach(t => {
        console.log(`Transceiver ${t.mid}:`,
            `Direction: ${t.direction}`,
            `Type: ${t.receiver.track.kind}`,
            `State: ${t.currentDirection}`
        );
        });
      ...
      openaiPC.ontrack = (e) => {
        ...
        const verifyAudioTracks = async (stream: MediaStream) => {
            console.log(`Verifying stream ${stream.id}...`);
          
            // Wait briefly for tracks to initialize (critical for OpenAI streams)
            await new Promise(resolve => setTimeout(resolve, 300));
          
            const audioTracks = stream.getAudioTracks();
            
            if (audioTracks.length === 0) {
              console.error("No audio tracks detected in stream:", {
                streamId: stream.id,
                active: stream.active,  // Correct MediaStream property
                // Removed invalid currentTime
                label: stream.id,       // More descriptive
                trackStates: []         // Explicit empty state
              });
              return false;
            }
            ...
            const trackInfo = audioTracks.map(track => ({
              ...
              muted: track.muted,
              readyState: track.readyState,  // "live", "ended", or "none"
              kind: track.kind,
              constraints: track.getConstraints(),
              // MediaStreamTrack-specific properties:
              isolated: track.isolated,
              contentHint: track.contentHint
            }));
          
            console.log(`Stream ${stream.id} track states:`, trackInfo);
          
            // Event listeners for state changes
            audioTracks.forEach(track => {
              const logChange = (event: string) => {
                console.log(`Track ${track.id} ${event}:`, {
                  enabled: track.enabled,
                  muted: track.muted
                });
              };
          
              track.addEventListener('mute', () => logChange('muted'));
              track.addEventListener('unmute', () => logChange('unmuted'));
              track.addEventListener('ended', () => {
                console.warn(`Track ${track.id} ended!`);
                // Consider automatic reconnection here
              });
            });
          
            return audioTracks.some(t => t.readyState === 'live');
          };
                ...
                if (!attendeeConnection) {
                    ...
                    if (openaiConn) {
                        ...
                        console.log(`Ready to forward audio from OpenAI to attendee ${attendeeId}`);
                         ...
                         //Attendee audio message handling. Now in ArrayBuffer format
                         attendeeConnection.dc.onmessage = async (event) => {
                             try {
                                 const audioContext = new AudioContext();
                                 ...
                                 //sendThroughDataChannel(attendeeConnection.dc, "audio", event.data as ArrayBuffer, { isBinary: true });
                             } catch (error) {
                                 console.error(`Error processing message from guide for ${language}:`, error);
                             }
                         };

                        // ICE handling for attendee
                        attendeeConnection.pc.onicecandidate = async (event) => {
                            if (event.candidate) {
                                try {
                                    await fetch("/api/tour/ice-candidate", {
                                        method: "POST",
...
while (reconnectAttempt <= maxReconnectAttempts) {
    ...
    try {
        ...
        //Add Cleanup for Failed Connections
        navigator.mediaDevices.getUserMedia({ audio: true })
            .then(stream => {
                stream.getTracks().forEach(t => t.stop());
                console.log("Cleaned up previous media tracks");
            })
            .catch(console.error);
...
async function sendClosingMessage(language: string): Promise<void> {
    ...
    if (openaiConn && openaiConn.dc.readyState === "open") {
      try {
        // Use a supported message type instead
        const message = {
          type: "session.update", // Valid type
          state: "closing" // Custom property
        };
        openaiConn.dc.send(JSON.stringify(message));
        
        // Simple timeout without all the event listeners
        await new Promise(resolve => setTimeout(resolve, 1000));
      } catch (error) {
...
/**
* Helper function to forward audio to attendees, now in ArrayBuffer format
*/
async function forwardAudioToAttendees(language: string, audioBuffer: ArrayBuffer): Promise<void> {
const connections = attendeeConnectionsByLanguage.get(language);
if (!connections || connections.size === 0) {
    console.log(`No attendees to forward audio for ${language}`);
    return;
}

console.log(`Forwarding audio to ${connections.size} attendees for ${language}`);

for (const conn of connections) {
    if (conn.dc.readyState === "open") {
        try {
            sendThroughDataChannel(conn.dc, "audio", audioBuffer, { isBinary: true });
        } catch (error) {
            console.error(`Error forwarding audio to attendee ${conn.id}:`, error);
        }
    }
}
}
